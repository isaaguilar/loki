...
func dataobjV2StoreWithOpts(dataDir string, tenantID string, cfg engine.ExecutorConfig, metastoreCfg metastore.Config) (*DataObjV2EngineStore, error) {
	...
	sched := &engine.Scheduler{Logger: logger}
	schedService, err := sched.Service()
	if err != nil {
		return nil, fmt.Errorf("creating scheduler service: %w", err)
	} else if err := schedService.StartAsync(context.Background()); err != nil {
		return nil, fmt.Errorf("starting scheduler service: %w", err)
	} else if err := schedService.AwaitRunning(context.Background()); err != nil {
		return nil, fmt.Errorf("awaiting scheduler service to run: %w", err)
	}
	...
	workerService, err := worker.Service()
	if err != nil {
		return nil, fmt.Errorf("creating worker service: %w", err)
	} else if err := workerService.StartAsync(context.Background()); err != nil {
		return nil, fmt.Errorf("starting worker service: %w", err)
	} else if err := workerService.AwaitRunning(context.Background()); err != nil {
		return nil, fmt.Errorf("awaiting worker service to run: %w", err)
	}
